...
def index():
    ...
    posts = current_user.followed_posts().paginate(                         # page arg == what page i'm?
        page, app.config['POSTS_PER_PAGE'], False)                          # app.config arg == how much posts per page i have
                                                                            # False arg is to not return 404 error when page is out of range    
...
def explore():
    page = request.args.get('page', 1, type=int)
    posts = Post.query.order_by(Post.timestamp.desc()).paginate(            # same args than index page  
...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    posts = user.posts.order_by(Post.timestamp.desc()).paginate(            # same args than index page
        page, app.config['POSTS_PER_PAGE'], False)
